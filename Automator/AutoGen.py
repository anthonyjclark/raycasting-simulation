...
class Navigator:
    ...
    def __init__(self, img_dir, maze, save_images=False):
    ...
    class State:
        ...
        def __init__(self, c_targ_x, c_targ_y, base_dir, targ_dir, world, img_dir=None, show_freq=None):
            ...
            self.dist = self.get_l2_dist()
            self.direction = self.get_rand_angle()

            self.img_dir = img_dir
            if self.img_dir != None:
                self.img_num_l = len(os.listdir(os.join.path(img_dir, 'left')))
                self.img_num_r = len(os.listdir(os.join.path(img_dir, 'right')))
                self.img_num_l = len(os.listdir(os.join.path(img_dir, 'straight')))
        ...
        def get_rand_angle(self):
            ...
            return rng.normal(loc=theta, scale=rand_angle_scale) % (2 * pi)
        ...
        def turn_to_angle(self, angle):
            ...
            i = 0
            while self.abs_angle_diff(angle) > .1:
                if self.turn_right(angle):

                    # save image right
                    if self.img_dir != None:
                        self.world.savePNG(f"{self.img_dir}/right/{self.img_num_r:05}.png")
                        self.img_num_r += 1
                    ...
                    if self.img_dir != None:
                        self.world.savePNG(f"{self.img_dir}/left/{self.img_num_l:05}.png")
                        self.img_num_l += 1
        ...
        def move_dist(self):
            ...
            while self.dist > 0.4:
                ...
                if self.img_dir != None:
                    self.world.savePNG(os.path.join(img_dir, 'straight', f"{self.img_num_s:05}.png"))


    def navigate(self, index, show_dir=False):
        _, _, s_base_dir = self.directions[i].split()
        targ_x, targ_y, s_targ_dir = self.directions[i + 1].split()
        targ_x, targ_y = int(targ_x), int(targ_y)
        curr_x, curr_y = self.world.getX(), self.world.getY()
