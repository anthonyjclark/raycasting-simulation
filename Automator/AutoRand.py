...
def get_angle(curr_x, curr_y, targ_x, targ_y):
    ...
    # case where target pos is down and to the right
        ...
        if curr_y == targ_y:
            ...
            theta = (atan((targ_x - curr_x) / (curr_y - targ_y))) % (
                2 * pi
            ) + 3 * pi / 2
...
def head_near_bound(curr_x, curr_y, targ_x, targ_y, angle, base_dir, targ_dir):
    ...
    if base_dir == 180:
        ...
        if pi < angle < 2 * pi and (curr_y - targ_y) < 0.1:
            return True
        elif (0 <= angle <= pi / 2 or (3 * pi / 2 <= angle <= 2 * pi)) and abs(
            targ_x + 1 - curr_x
        ) < 0.2:
            return targ_dir != 0
        else:
...
def keep_straight(curr_x, curr_y, targ_x, targ_y, angle, base_dir, targ_dir, step):
    ...
    if (
        head_near_bound(curr_x, curr_y, targ_x, targ_y, angle, base_dir, targ_dir)
        and dist < 1.0
    ):
        return False
    elif step < 0.15:
...
def main():
    ...
    while i < len(directions) - 1:
        ...
        while abs_dist > 0.4:
            ...
            if abs_dist < 1.0 and not check_past(
                curr_x, curr_y, c_targ_x, c_targ_y, base_dir, targ_dir
            ):
                rand_angle = angle_correct(
                    curr_x, curr_y, c_targ_x, c_targ_y, base_dir, targ_dir
                )
            else:
            ...
            move_straight = keep_straight(
                curr_x, curr_y, targ_x, targ_y, curr_dir, base_dir, targ_dir, step
            )
