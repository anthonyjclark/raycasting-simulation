...
model_names = ["rnet18.pth", "rnet34.pth", "rnet50.pth", "rnet101.pth", "xrnet18.pth", "xrnet34.pth", "xrnet50.pth", "xrnet101",
...
def main()
    ...
    pretrained = bool(sys.argv[2]) if len(sys.argv[2] > 1 else False
    file_name = sys.argv[3] if len(sys.arg[3] > 1 else "resnet18.pth"
    ...
    sub_dir = str(model_names[i]).split('.')[0]
    path = os.path.abspath(os.path.join("classification_models", sub_dir))
    os.mkdir(path)
    
    # Create and finetune model
    learn = cnn_learner(dls, model_archs[i], loss_func=CrossEntropyLossFlat(), metrics=accuracy)
    learn.fine_tune(5)
    ...
    # Export Model
    learn.export(os.path.join(path, model_names[i]))
